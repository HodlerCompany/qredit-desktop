;(function () {
  ...
  function NetworkService ($q, $http, $timeout, storageService, timeService, toastService) {
    ...
    const firstQreditJsNetworkKey = 'qredit'
    ...
    ark.crypto.setNetworkVersion(network.version || 0x3a)
    ...
    function switchNetwork (newnetwork, reload) {
      ...
      if (!n) {
        n = {
          'qredit': createNetworkFromQreditJs(firstQreditJsNetworkKey, 0x3a, 1, 'url(assets/images/images/Qredit.jpg)')
        }
        storageService.setGlobal('networks', n)
      }
      if (reload) {
    ...
    function tryGetPeersFromQreditJs () {
      ...
      const qreditjsNetwork = ark.networks[network.qreditJsKey]
      if (!qreditjsNetwork) {
        return
      }
    ...
    function getLatestClientVersion () {
      const deferred = $q.defer()
      const url = 'https://api.github.com/repos/HodlerCompany/qredit-desktop/releases/latest'
